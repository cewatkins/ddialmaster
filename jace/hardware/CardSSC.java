...
public class CardSSC extends Card implements Reconfigurable {
    ...
    public short IP_PORT = (short) (1977 + getSlot());
    ...
    private boolean FULL_ECHO = false;
...
//    private boolean TRANS_ADD_LF = true;
    ...
    private boolean telnetNegotiated = false;
        ...
        while (socket != null && !socket.isClosed()) {
            try {
                ...
                while ((clientSocket = socket.accept()) != null) {
                    ...
                    // Telnet negotiation: suppress echo and go-ahead
                    OutputStream outStream = clientSocket.getOutputStream();
                    outStream.write(new byte[] { (byte)IAC, (byte)WILL, (byte)SUPPRESS_GO_AHEAD });
                    outStream.write(new byte[] { (byte)IAC, (byte)WILL, (byte)ECHO });
                    outStream.flush();
    ...
    // Telnet input handling: skip telnet commands, only deliver data bytes
    ...
    // Removed duplicate getInputByte() implementation

    // Telnet output: escape IAC bytes, do not echo input
    protected void sendOutputByte(int i) throws IOException {
        if (clientSocket != null && clientSocket.isConnected()) {
            ...
            if (b == IAC) {
                outStream.write(IAC);
                outStream.write(IAC); // Escape IAC
            } else {
                outStream.write(b);
            }
            if (TRANS_ADD_LF && b == 13) {
                outStream.write(10);
            }
            outStream.flush();
...
// Removed duplicate inputAvailable() method to fix compilation errors.
